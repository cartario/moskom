...
export default ({password, touched}) => {  
  ...
  return (
    <div className="rules">
      ...
      <ul >
        <li className={          
          password.length < 2 &&!touched ? `default` : getMessage(password, passwordSize)}>
          От 6 до 32 символов</li>
        <li className={         
          !hasDifLetters && !hasNumber && !touched ? `default` :  getMessage(password, hasNumber)}>
          Цифру
        </li>
        <li className={         
          password.length < 2 &&!touched? `default` : getMessage(password, hasDifLetters)}>
